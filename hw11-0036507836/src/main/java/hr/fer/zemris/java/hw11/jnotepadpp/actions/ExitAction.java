...
/**
 * Action to exit the whole application
 * @author Matija
 *
 */
public class ExitAction extends LocalizableAction{
	 ...
	 */
	public ExitAction(String key, ILocalizationProvider lp, KeyStroke acceleratorKey, int mnemonicKey, String shortDescription, MultipleDocumentModel documents, JFrame frame) {
		super(key, lp);
		this.lp = lp;
	...
	public void actionPerformed(ActionEvent e) {
		while(documents.getNumberOfDocuments()>0) {
			SingleDocumentModel doc = documents.getDocument(documents.getNumberOfDocuments()-1);
			...
			if(doc.isModified()) {
				...
				int selected = JOptionPane.showConfirmDialog(null, msg, "Save", 
