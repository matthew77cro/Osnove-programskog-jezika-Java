...
public class Util {
	...
	protected static void saveFile(ActionContext context, Path p) {
		...
		for(int i=0; i<context.getModel().getSize(); i++) {
			...
			} else if(obj instanceof FilledTriangle) {
				FilledTriangle l = (FilledTriangle) obj;
				sb.append(String.format("FTRIANGLE %d %d %d %d %d %d %d %d %d %d %d %d%n",
						l.getX0(), l.getY0(), l.getX1(), l.getY1(), l.getX2(), l.getY2(), l.getOuterColor().getRed(),
						l.getOuterColor().getGreen(), l.getOuterColor().getBlue(),
						l.getFillColor().getRed(), l.getFillColor().getGreen(),
						l.getFillColor().getBlue()));
			}
