...
class SmartScriptParserTestDocument1 {
	...
	static void setUpBeforeClass() throws Exception {
		...
		String docBody = new SmartScriptParserTestDocument1().loader("document1.txt");
	...
	void testForTagElements() {
		...
		fln = (ForLoopNode) docNode.getChild(3);
		assertEquals("i", fln.getVariable().asText());
		assertEquals("0", fln.getStartExpression().asText());
		assertEquals("10", fln.getEndExpression().asText());
		assertEquals("2", fln.getStepExpression().asText());
	}
	
	@Test
	void testEchoTagElements() {
		EchoNode en = (EchoNode) docNode.getChild(3).getChild(3);
		assertEquals(6, en.getElements().length);
		...
		assertEquals("decfmt", en.getElements()[5].asText());
	}
